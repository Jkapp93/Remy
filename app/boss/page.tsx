...
type Conversation = { id: string; summary: string; created_at: string; job_id: string };
type Doctrine = { id: string; content: string; type: string; created_at: string };
type Profile = { id: string; clerk_id: string; full_name: string; email: string; role: string; created_at: string };
type Invite = { id: string; email: string; role: string; token: string; accepted: boolean; created_at: string };
...
export default function BossDashboard() {
  ...
  const loadAll = async () => {
    ...
    const [jobData, convData, docData, profileData, inviteData] = await Promise.all([
              ...
              <div style={{ background:'#111820', border:'1px solid rgba(255,255,255,0.07)', borderRadius:'12px', padding:'20px' }}>
                ...
                {conversations.map(conv => (
                  <div key={conv.id} className="data-row" style={{ flexDirection:'column', alignItems:'flex-start', gap:'6px' }}>
                    <div style={{ fontSize:'0.85rem', color:'#e8edf2', lineHeight:1.6 }}>{conv.summary || 'No summary available'}</div>
                    <div style={{ fontSize:'0.62rem', color:'#2d3f52' }}>
                      {new Date(conv.created_at).toLocaleDateString('en-US', { month:'short', day:'numeric', hour:'numeric', minute:'2-digit' })}
                    </div>
                  </div>
                ))}
                {conversations.length === 0 && <div style={{ color:'#3d5268', fontSize:'0.82rem' }}>No conversations logged yet.</div>}
              </div>
            )}
                ...
                {doctrine.map(doc => (
                  <div key={doc.id} className="data-row">
                    <div style={{ flex:1 }}>
                      ...
                      <div style={{ fontSize:'0.82rem', color:'#7a8fa4', fontWeight:300, lineHeight:1.5 }}>{doc.content.slice(0, 120)}{doc.content.length > 120 ? '...' : ''}</div>
                    </div>
                    <button onClick={() => removeDoc(doc.id)} style={{ background:'transparent', border:'none', color:'#2d3f52', cursor:'pointer', fontSize:'0.9rem' }}>x</button>
                  </div>
                ))}
            ...
            {activeTab === 'team' && (
              <div style={{ display:'flex', flexDirection:'column', gap:'16px' }}>
                ...
                <div style={{ background:'#111820', border:'1px solid rgba(255,255,255,0.07)', borderRadius:'12px', padding:'20px' }}>
                  ...
                  {profiles.length === 0 && <div style={{ color:'#3d5268', fontSize:'0.82rem' }}>No team members yet. Send an invite above.</div>}
                </div>
