...
export default function VoicePage() {
  ...
  useEffect(() => {
    if (messages.length === 0) {
      setMessages([{ role: 'assistant', content: `Hey${user?.firstName ? ` ${user.firstName}` : ''}. I'm Remy. Tell me about your current job â€” customer name, address, what you're walking into â€” and I'll brief you before you knock.` }]);
    }
  }, [user]);
        ...
        {messages.map((m, i) => (
          <div key={i} style={{ display: 'flex', justifyContent: m.role === 'user' ? 'flex-end' : 'flex-start' }}>
            <div style={{
              ...
              background: m.role === 'user' ? '#1c2a38' : 'rgba(240,122,46,0.08)',
              border: m.role === 'user' ? '1px solid rgba(255,255,255,0.08)' : '1px solid rgba(240,122,46,0.2)',
              fontSize: '0.9rem',
              lineHeight: 1.6,
      ...
      <div style={{ padding: '20px 24px', borderTop: '1px solid rgba(255,255,255,0.07)', background: 'rgba(11,15,20,0.95)', backdropFilter: 'blur(20px)' }}>
        <div style={{ maxWidth: '720px', margin: '0 auto', display: 'flex', gap: '10px', alignItems: 'center' }}>
          ...
          <input
            ...
            style={{ flex: 1, background: '#111820', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '24px', padding: '13px 20px', color: '#e8edf2', fontFamily: "'DM Sans', sans-serif", fontSize: '0.9rem', outline: 'none' }}
          />
          <button onClick={() => send(input)} style={{ padding: '13px 20px', background: '#f07a2e', border: 'none', borderRadius: '24px', color: '#fff', fontFamily: "'DM Sans', sans-serif", fontSize: '0.85rem', fontWeight: 500, cursor: 'pointer' }}>
            Send
          </button>
        </div>
      </div>
