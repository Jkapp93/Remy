...
export default function VoicePage() {
  ...
  useEffect(() => {
    if (!user) return;
    loadContext().then(() => {
      setMessages([{ role: 'assistant', content: `Hey${user?.firstName ? ` ${user.firstName}` : ''}. I am Remy. Select a job from the top and I will brief you before you knock. Or just tell me what you are walking into.` }]);
      setReady(true);
    });
  }, [user]);
  ...
  const send = useCallback(async (text: string, currentMessages: {role: string; content: string}[], currentDoctrine: string, currentJob: {id: string; customer_name: string; address: string; notes: string} | null) => {
  ...
  const startListening = () => {
    ...
    recognition.onresult = (e: any) => {
      ...
      setListening(false);
      setMessages(prev => {
        send(t, prev, doctrine, activeJob);
        return prev;
      });
    };
  ...
  const selectJob = (job: {id: string; customer_name: string; address: string; notes: string}) => {
    ...
    const briefPrompt = `I am pulling up to ${job.customer_name}${job.address ? ` at ${job.address}` : ''}${job.notes ? `. Notes: ${job.notes}` : ''}. Brief me fast before I knock.`;
    setMessages(prev => {
      send(briefPrompt, prev, doctrine, job);
      return prev;
    });
  };
    ...
    <div style={{ background: '#0b0f14', height: '100dvh', color: '#e8edf2', fontFamily: "'DM Sans', sans-serif", display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      <style>{`
        ...
        * { box-sizing:border-box; margin:0; padding:0; }
        ...
        .mic-wrap { position:relative; width:56px; height:56px; flex-shrink:0; }
        .mic-btn { width:56px; height:56px; border-radius:50%; display:flex; align-items:center; justify-content:center; cursor:pointer; border:none; font-size:1.3rem; transition:all 0.2s; position:relative; z-index:1; }
        ...
        .mic-speaking { background:rgba(61,175,118,0.15); border:1.5px solid rgba(61,175,118,0.5); }
      ...
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'12px 16px', borderBottom:'1px solid rgba(255,255,255,0.06)', background:'rgba(11,15,20,0.98)', flexShrink:0, gap:'10px' }}>
        ...
        <div style={{ display:'flex', alignItems:'center', gap:'8px' }}>
          {speaking && <span style={{ fontSize:'0.68rem', color:'#3daf76', fontWeight:600, letterSpacing:'0.1em', textTransform:'uppercase' }}>Speaking</span>}
          {listening && <span style={{ fontSize:'0.68rem', color:'#f07a2e', fontWeight:600, letterSpacing:'0.1em', textTransform:'uppercase' }}>Listening</span>}
        ...
        <div style={{ background:'#111820', borderBottom:'1px solid rgba(255,255,255,0.06)', padding:'14px 16px', flexShrink:0 }}>
          ...
          {jobs.length === 0 ? (
            ...
            <div style={{ display:'flex', flexDirection:'column', gap:'6px' }}>
              {jobs.map(job => (
                <div key={job.id} onClick={() => selectJob(job)} style={{ padding:'10px 14px', background:'#0b0f14', border:'1px solid rgba(255,255,255,0.05)', borderRadius:'8px', cursor:'pointer' }}>
      ...
      <div style={{ flex:1, overflowY:'auto', padding:'16px', display:'flex', flexDirection:'column', gap:'14px' }}>
        {messages.map((m, i) => (
          <div key={i} style={{ display:'flex', justifyContent: m.role === 'user' ? 'flex-end' : 'flex-start', alignItems:'flex-end', gap:'8px' }}>
            {m.role === 'assistant' && (
              <div style={{ width:'26px', height:'26px', borderRadius:'50%', background:'rgba(240,122,46,0.15)', border:'1px solid rgba(240,122,46,0.3)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:'0.65rem', flexShrink:0, color:'#f07a2e', fontWeight:700 }}>R</div>
            )}
      ...
      <div style={{ padding:'12px 16px', borderTop:'1px solid rgba(255,255,255,0.06)', background:'rgba(11,15,20,0.98)', flexShrink:0, paddingBottom:'max(12px, env(safe-area-inset-bottom))' }}>
        <div style={{ display:'flex', gap:'10px', alignItems:'center' }}>
          <div className="mic-wrap">
            <button className={`mic-btn ${listening ? 'mic-listening' : speaking ? 'mic-speaking' : 'mic-idle'}`} onClick={startListening}>
              {listening ? 'stop' : speaking ? 'mute' : 'mic'}
            </button>
            {listening && <div className="ripple-ring"></div>}
          </div>
          <input className="msg-input" value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => { if (e.key === 'Enter') { setMessages(prev => { send(input, prev, doctrine, activeJob); return prev; }); }}} placeholder={listening ? 'Listening...' : 'Type or tap mic...'} disabled={listening} />
          <button className="send-btn" onClick={() => { setMessages(prev => { send(input, prev, doctrine, activeJob); return prev; }); }} disabled={loading || listening || !input.trim()}>Send</button>
        </div>
      </div>
    </div>
  );
