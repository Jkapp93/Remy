      ...
      <div style={{ maxWidth: '720px', margin: '0 auto', padding: '32px 24px' }}>
        ...
        {showNew && (
          <div style={{ background: '#111820', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '12px', padding: '24px', marginBottom: '20px' }}>
            ...
            <textarea value={notes} onChange={e => setNotes(e.target.value)} placeholder="Notes (what you know about this job)" rows={3} style={{ width: '100%', background: '#0b0f14', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '8px', padding: '10px 14px', color: '#e8edf2', fontFamily: "'DM Sans', sans-serif", fontSize: '0.9rem', outline: 'none', resize: 'vertical', marginBottom: '14px' }} />
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
            {jobs.map((job: any) => (
              <div key={job.id} style={{ background: '#111820', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '12px', padding: '18px 20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                <Link href={`/dashboard/voice?job=${job.id}`} style={{ background: 'rgba(240,122,46,0.1)', border: '1px solid rgba(240,122,46,0.2)', color: '#f07a2e', padding: '8px 16px', borderRadius: '6px', textDecoration: 'none', fontSize: '0.78rem', fontWeight: 500 }}>Talk to Remy â†’</Link>
              </div>
            ))}
          </div>
        )}
